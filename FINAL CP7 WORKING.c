...
Problem is to convert the grocery program into the same thing but using External Binary Files, mainly a master and temp file for editing and deleting.
Date Created: May 4, 2025
Data Finished: May 8, 2025
*/
...
void deleteRecord(FILE *fp, FILE *tempf, GROCERY record);
void typeDelete(FILE *fp, FILE *tempf, GROCERY record);
void nameDelete(FILE *fp, FILE *tempf, GROCERY record);
...
void nameSearch(FILE *fp, FILE *tempf, GROCERY record){
	...
	while (fread(&record, sizeof(GROCERY), 1, fp)) {
		searchcheck = strcmpi(record.itemName, search);
		if (searchcheck == 0) {
			printf("%-15d%-15s%-15s%-15s%-15.2f%-15d\n", record.itemCode, record.itemName, record.manufacturer, record.productType, record.price, record.currentStock);
			toModify = record.itemCode;
			found += 1;
...
void deleteRecord(FILE *fp, FILE *tempf, GROCERY record) {
	...
	do {
		...
		switch (choice) {
			case 1:
				typeDelete(fp, tempf, record);
				break;
			case 2:
				nameDelete(fp, tempf, record);
				break;
			default:
				printf("Invalid Option, press [Enter] to retry");
				getch();
